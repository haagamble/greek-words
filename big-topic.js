export const bigTopics = [
  {
    id: "verbs-1",
    label: "Verbs 1",
    categories: ["verbs"],
    split: { verbs: "first" },
  },
  {
    id: "verbs-2",
    label: "Verbs 2",
    categories: ["verbs"],
    split: { verbs: "second" },
  },
  {
    id: "descriptions-1",
    label: "Descriptions 1",
    categories: ["adjectives", "colours", "comparatives"],
    split: { adjectives: "first", colours: "first", comparatives: "first" },
  },
  {
    id: "descriptions-2",
    label: "Descriptions 2",
    categories: ["adjectives", "colours", "comparatives"],
    split: { adjectives: "second", colours: "second", comparatives: "second" },
  },
  {
    id: "ideas-1",
    label: "Ideas 1",
    categories: ["abstract-nouns"],
    split: { "abstract-nouns": "first" },
  },
  {
    id: "ideas-2",
    label: "Ideas 2",
    categories: ["abstract-nouns", "law-rules"],
    split: { "abstract-nouns": "second" },
  },
  {
    id: "food-1",
    label: "Food 1",
    categories: ["food-drink"],
    split: { "food-drink": "first" },
  },
  {
    id: "food-2",
    label: "Food 2",
    categories: ["food-drink", "spices-seasonings"],
    split: { "food-drink": "second" },
  },
  {
    id: "home",
    label: "Home",
    categories: ["household", "rooms", "house-parts", "appliances"],
  },
  {
    id: "tools-tech",
    label: "Tools & Tech",
    categories: ["tools-objects", "technology", "materials", "elements"],
  },
  {
    id: "body-care",
    label: "Body & Care",
    categories: ["body-health", "personal-care"],
  },
  {
    id: "animals-plants",
    label: "Animals & Plants",
    categories: ["animals", "flowers"],
  },
  {
    id: "nature-weather",
    label: "Nature & Weather",
    categories: ["nature", "nature-places", "places", "weather", "seasons"],
  },
  {
    id: "travel",
    label: "Travel",
    categories: ["geography", "road-travel", "travel", "vehicles"],
  },
  {
    id: "number-measure",
    label: "Number & Measure",
    categories: ["numbers", "ordinals", "measurements", "shapes", "time-expressions", "time-units", "days-week"],
  },
  {
    id: "people-words",
    label: "People & Language",
    categories: ["people-family", "people-roles", "communication"],
  },
  {
    id: "work-school",
    label: "Work & School",
    categories: ["professions", "school-office", "school-subjects"],
  },
  {
    id: "grammar",
    label: "Grammar",
    categories: ["common-phrases", "question-words", "function-words", "prepositions", "pronouns", "adverbs"],
  },
  {
    id: "shopping-style",
    label: "Shopping & Style",
    categories: ["money-shopping", "clothing", "precious-stones"],
  },
  {
    id: "sports-music",
    label: "Sports & Music",
    categories: ["sports", "musical-instruments", "toys"],
  },
];
